...
/** Burn ONE image (watermark / logo bug) over the composed output during
 * the single render pass — ONE ffmpeg `overlay` filter fed by an ADDITIONAL
 * image input (the audio-mix second-input precedent), anchoring on the
 * OUTPUT frame (after zoom/scale — the watermark never zooms with the
 * content). PNG-with-alpha is the documented contract; `format=rgba` on the
 * overlay input only is what carries the alpha over the yuv main stream. */
export const ImageOverlayOp = z.object({
  type: z.literal("image-overlay"),
  /** image file; png with alpha is the documented contract (must exist). */
  file: z.string().min(1),
  /** placement; default "bottom-right" (render layer applies it) */
  position: z
    .enum(["top-left", "top-right", "bottom-left", "bottom-right", "center"])
    .optional(),
  /** overlay width in px (int, resize/export-gif mirror); scales the overlay
   * KEEPING aspect (`scale=W:-1`); absent = native size. Height is NOT
   * even-constrained: only the encoded MAIN stream needs -2. */
  width: z.number().int().gt(0).max(16384).optional(),
  /** opacity 0 < o ≤ 1 (default 1) — kept RAW here (the zoom.factor
   * pattern) so validate can emit its own OPERATION_INVALID fence instead
   * of a generic Zod rejection. */
  opacity: z.number().optional(),
  /** visibility window on the OUTPUT timeline (s); both absent = always
   * visible (overlay-text semantics). */
  from: z.number().min(0).optional(),
  to: z.number().min(0).optional(),
});
