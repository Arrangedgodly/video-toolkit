/** Committed `ffmpeg -h filter=xfade` fixture (T13) — captured verbatim 2026-09-03
 *  from THIS build (ffmpeg 9.0.1 ffmpeg-full, stdout of
 *  `ffmpeg -hide_banner -h filter=xfade`, trailing "Exiting" notice included).
 *  Unit tests parse THIS text; no live ffmpeg in unit tests. The live
 *  integration test asserts this build still parses to the same catalog —
 *  a version upgrade that changes the enum should fail that test loudly
 *  and refresh this fixture. */
export const XFADE_HELP_FIXTURE = `
Filter xfade
