...
STATUS: milestones M1–M10 complete (M6 bridges, M7 audio-mix single-pass, M8 review-frames + transcribe concurrency, M9 VTT + ship gate, M10 follow-up closure) · M11 ACTIVE (T14 overlay-text done; T10 whisper-cpp engine + word timestamps done; T11 filler precision done) · 222 tests (`npm test`, ~25 s; transcription tests skip without an engine) · binary `video` on PATH (npm link) · node ≥ 20, ffmpeg/ffprobe required.
...
Zod source of truth: `src/core/schemas.ts`. Shapes: SilenceReport `{segments:[{start,end,duration}], duration?, note?, params?}` · SceneReport `{boundaries:[{timestamp,confidence}]…}` · TranscriptReport `{segments:[{start,end,text,words?:[{start,end,text}]}], engine?, model?, language?, params?}` (`words` present only from whisper-cpp `--word-timestamps` — absence means segment granularity only; `params.wordTimestamps` records the flag on the native path; windowed params carry `chunkSeconds`/`snapToSilence` instead) · FillerReport `{instances:[{start,end,phrase,context}], params?{phrases, precision}}` (`precision: "words"` = exact anchors from `segments[].words`, `"segments"` = linear interpolation; `"words"` only when every segment carries words; phrases never match across segments) · HighlightReport `{candidates:[{start,end,score,text,reasons[]}], params?}` (score = weighted speech-rate 0.45 / pause-before 0.2 / keyword 0.25 / length-band 0.1, renormalized when keywords unset; deterministic proposal — the agent decides).
