...
| `transcribe` | `<input>` | `--engine handy --model <id> --chunk <s=25> --no-snap --concurrency <n>` | TranscriptReport — windows run as bounded parallel jobs (n at a time; default = cached `video benchmark` renderConcurrency recommendation for this source+ffmpeg build when it is an integer in 1..4, else 1); report byte-identical to sequential (segments re-sorted by window start; concurrency NOT in the cache key — warm cache hits at any n); non-integer/<1 n → `OPERATION_INVALID`; any window failure fails the whole task (`TRANSCRIPTION_ENGINE_FAILED`) | transcript-<engine>-<model>-c<chunk>.json |
