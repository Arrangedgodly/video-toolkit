...
3. Decide: turn observations into editorial choices. `video plan <input> --cuts-from silence.json` pre-fills silence cuts deterministically; `video plan <input> --highlights-from highlights.json` scaffolds a best-moments compilation from a `find-highlights` report. Keep, drop, or tune the proposed ops by judgment.
